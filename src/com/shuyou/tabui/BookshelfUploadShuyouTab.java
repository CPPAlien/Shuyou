...
public class BookshelfUploadShuyouTab extends Activity{
	
	private final static String TAG = "BookshelfUpLoadShuyouTab";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.shuyou_list_booksupload);
		
		loading = (RelativeLayout) this.findViewById(R.id.list_booksupload_loading_layout);
        new AnimationUtil().showLoadingDialog(loading);
		
		refreshList = (PullToRefreshListView)findViewById(R.id.list_bookshelfupload);
		...
		refreshList.setOnRefreshListener(new OnRefreshListener() {
			...
			public void onRefresh() {
				// TODO Auto-generated method stub
				getMyBookshelf("my_book", "");   //刷新请求第一页
			}
		...
		BookshelfListViewAdapter tmpAdapter = new BookshelfListViewAdapter(BookshelfUploadShuyouTab.this, 
				booklist, tmpList, list, "upload");
		for(int i = 0; i < 4; ++i) {
	...
	private void LoadingSuccess(Data tData) {
		...
		BookshelfListViewAdapter bookshelfViewAdapter = new BookshelfListViewAdapter(BookshelfUploadShuyouTab.this, 
				booklist, bookItemList, list, "upload");
		list.setDivider(null);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode == RESULT_OK){
			String str = data.getExtras().getString("bookId");
			RelativeLayout position = (RelativeLayout)list.findViewWithTag(str);
			position.setVisibility(View.GONE);
			Toast.makeText(getApplicationContext(), str+"我执行了", Toast.LENGTH_SHORT).show();
			LogHelper.i("uploadtag", str+"我执行了");
		}
	}
	...
	static class MyInnerHandler extends Handler {
        WeakReference<BookshelfUploadShuyouTab> mActivity;

        MyInnerHandler(BookshelfUploadShuyouTab mAct) {
        	mActivity = new WeakReference<BookshelfUploadShuyouTab>(mAct);
        }

        @Override
        public void handleMessage(Message msg) {
        	BookshelfUploadShuyouTab theAct = mActivity.get();
            switch (msg.what) {

...
public class BookshelfBorrowShuyouTab extends Activity{

	private String TAG = "BookshelfBorrowShuyouTab";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.shuyou_list_booksborrow);
		
		loading = (RelativeLayout) this.findViewById(R.id.list_booksborrow_loading_layout);
        new AnimationUtil().showLoadingDialog(loading);
		
		refreshList = (PullToRefreshListView)findViewById(R.id.list_bookshelfborrow);
		...
		refreshList.setOnRefreshListener(new OnRefreshListener() {
			...
			public void onRefresh() {
				// TODO Auto-generated method stub
				getMyBookshelf("borrow_book", "");   //刷新请求第一页
			}
		});
		...
		BookshelfListViewAdapter tmpAdapter = new BookshelfListViewAdapter(BookshelfBorrowShuyouTab.this, 
				booklist, tmpList, list, "borrow");
		for(int i = 0; i < 4; ++i) {
			BookshelfListView bookItem = new BookshelfListView("null", "null", "null");
			tmpList.add(bookItem);
		}
		list.setAdapter(tmpAdapter);	
		getMyBookshelf("borrow_book", "");
	}
	...
	private void LoadingSuccess(Data tData) {
		...
		BookshelfListViewAdapter bookshelfViewAdapter = new BookshelfListViewAdapter(BookshelfBorrowShuyouTab.this, 
				booklist, bookItemList, list, "borrow");
		list.setDivider(null);
	...
	static class MyInnerHandler extends Handler {
        WeakReference<BookshelfBorrowShuyouTab> mActivity;

        MyInnerHandler(BookshelfBorrowShuyouTab mAct) {
        	mActivity = new WeakReference<BookshelfBorrowShuyouTab>(mAct);
        }

        @Override
        public void handleMessage(Message msg) {
        	BookshelfBorrowShuyouTab theAct = mActivity.get();
            switch (msg.what) {
